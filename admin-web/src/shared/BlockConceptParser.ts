// Mirrored verbatim from src/lib/BlockConceptParser.ts — this file has zero
// RN dependencies, so it's copied byte-for-byte rather than reimplemented.
// program_blocks.notes encodes block metadata as `[CONCEPT:{json}] <notes>`;
// any drift between the mobile and web copies of this file would make
// blocks built on one platform render wrong (or silently lose settings) on
// the other. Keep in sync manually if the mobile file changes.
