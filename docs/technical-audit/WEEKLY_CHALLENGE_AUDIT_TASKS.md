...
- [x] **`getChallengeLeaderboard()` in `weeklyChallenge.ts` has no `.limit()`** — `src/lib/weeklyChallenge.ts:103-134`. The query has two `.order()` calls (`score` then `submitted_at`) but no `.limit()` — returns every entry for a challenge, unbounded. For a popular challenge with many participants, this grows with user count. Should be capped at 100 (or 50) consistent with other leaderboards.
