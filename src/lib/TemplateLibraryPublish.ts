...
// Runs client-side first for fast inline feedback, then the authoritative
// publish_library_template RPC re-checks the parts that matter for data
// integrity (tier range bounds, goal presence, duplicate coverage) before
// actually flipping status to 'published' — same "client pre-check for
// UX, server is authoritative" split TrialService.isTimeValid() uses for
// the time hard-floor (see CLAUDE.md).

import { supabase } from './supabase';
import { BlockConceptParser } from './BlockConceptParser';
import { TierRange } from './templateLibrary';
