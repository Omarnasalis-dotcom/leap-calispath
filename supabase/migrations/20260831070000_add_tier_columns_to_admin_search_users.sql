-- admin-web's user list has zero subscription-tier visibility today — the
-- app gained a colored tier badge earlier this session, but this RPC's
-- fixed RETURNS TABLE column list predates subscription_tier entirely.
-- Postgres forbids changing a RETURNS TABLE column list via CREATE OR
-- REPLACE (raises "cannot change return type of existing function"), so
-- the exact original signature must be dropped first — same rule as this
-- project's function-overload gotcha, just for a return-type change
-- instead of an argument-list change. Body otherwise unchanged from
-- 20260807290000.
DROP FUNCTION IF EXISTS public.admin_search_users(text, text, boolean, integer, integer);
...
CREATE OR REPLACE FUNCTION public.admin_search_users(p_query text DEFAULT NULL::text, p_sort text DEFAULT 'created_at'::text, p_desc boolean DEFAULT true, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, email text, display_name text, first_name text, last_name text, strength_tier integer, power_tier integer, statics_tier numeric, glory_score integer, power_points numeric, one_mm_points numeric, streak integer, is_admin boolean, is_coach boolean, community_id uuid, community_name text, country text, gender text, last_active timestamp with time zone, created_at timestamp with time zone, subscription_tier text, access_expires_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
...
BEGIN
  ...
  RETURN QUERY EXECUTE format(
    ...
    SELECT p.id, p.email, p.display_name, p.first_name, p.last_name,
           ...
           p.last_active, u.created_at, p.subscription_tier, p.access_expires_at,
