-- save_program_template wiped block_exercises for EVERY previously-existing
-- block up front, before determining which blocks would actually survive.
-- A block that fails to delete because of the workout_logs FK (caught below
-- as foreign_key_violation) is no longer present in the incoming p_blocks
-- payload either (the coach's client already dropped it from state when
-- they "deleted" it) — so its exercises were wiped with no path to ever
-- restore them: permanently empty on every subsequent save.
--
-- Fix: only pre-wipe exercises for blocks that are about to be repopulated
-- by the per-block loop below (v_current_block_ids). For blocks being
-- removed, delete their exercises in the same exception-guarded block as
-- the program_blocks delete attempt, so a caught FK violation rolls back
-- both statements together (PL/pgSQL's BEGIN...EXCEPTION implicitly
-- savepoints) and the block keeps both its row and its exercises intact.
