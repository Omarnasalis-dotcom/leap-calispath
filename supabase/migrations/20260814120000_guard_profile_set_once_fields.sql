-- L12 (full-repo audit, 2026-08-10): the "set once" rule for gender/country
-- was enforced only client-side, in EditProfileModal.tsx — it builds the
-- update payload as `if (!profile.gender && editGender) updates.gender = ...`,
-- so a direct API call (or a modified client) could still overwrite an
-- already-set gender/country. Not a privilege escalation (these are cosmetic
-- profile fields, not gated on anything), but the "set once" claim shown to
-- users wasn't actually backed by the DB. Extending the existing protected-
-- fields trigger to also reject changing gender/country away from a value
-- they're already set to, while still allowing the initial set from NULL.
