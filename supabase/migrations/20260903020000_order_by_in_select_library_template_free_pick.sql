-- Adds ORDER BY id to the "which template matches this tier range" lookup,
-- matching the identical ordering just added to getRecommendations()
-- client-side (src/lib/templateLibrary.ts). Body otherwise unchanged from
-- 20260901030000.
--
-- Without an explicit order, Postgres makes no guarantee about which row
-- LIMIT 1 returns if a tier range is ever matched by more than one
-- published template — today's 8-template catalog happens to have exactly
-- one match per range, so this has been dormant, but the client's
-- independent query (different SQL, different query plan, no shared
-- ordering) could then legitimately pick a different "free" template than
-- this function does, showing one as free on the card and rejecting it
-- with PRO_REQUIRED at confirm. Same fix, same column, both sides.
