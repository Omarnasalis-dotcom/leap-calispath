-- Freemium gating redesign (2026-08-28): Customize Program becomes
-- "browse and build free, pay to commit" — the per-item is_free open-time
-- lock is removed client-side (CustomizeProgramScreen.tsx), so every
-- workout is viewable/selectable for free. The paywall trigger moves
-- entirely to "Create Program": this RPC is Pro-only now, with no per-item
-- exception (unlike Program Templates, there is no free custom-program
-- allowance at all). Pro callers see byte-for-byte the same outcome as
-- before — every previously-passing case still passes.
