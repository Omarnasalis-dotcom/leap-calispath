-- handleSaveTemplate previously ran as a sequence of separate client-side
-- Supabase calls (upsert template, wipe all exercises for the template,
-- delete removed blocks, then loop-upsert each block + insert its
-- exercises). A failure partway through that sequence (network drop,
-- constraint violation on block N of M) left blocks with their exercises
-- already wiped but never reinserted. This wraps the whole save in one
-- transaction.
--
-- Deliberately SECURITY INVOKER (the default — no clause needed): existing
-- RLS policies on program_templates/program_blocks/block_exercises already
-- correctly scope writes to the owning coach, with "Admin manages all ..."
-- policies separately covering admin access. Running as the caller keeps
-- that enforcement intact rather than re-implementing ownership checks here.
--
-- Blocks removed by the coach that still have workout_logs attached can't
-- be deleted (FK constraint, ON DELETE without CASCADE) — previously this
-- error was silently swallowed. Now each delete is attempted individually
-- inside its own sub-transaction (savepoint), so one undeletable block
-- doesn't abort the rest of the save, and the ids that couldn't be removed
-- are returned to the caller instead of being silently dropped.
...
BEGIN
  ...
  IF p_template_id IS NOT NULL THEN
    ...
    IF v_previous_block_ids IS NOT NULL THEN
      DELETE FROM block_exercises WHERE block_id = ANY(v_previous_block_ids);
