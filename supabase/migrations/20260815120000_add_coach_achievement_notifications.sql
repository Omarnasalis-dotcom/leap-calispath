-- Coach/admin notification inbox, part 1: server-side.
--
-- Every PB/tier-up notification today (tier_promotion, power_pb, static_pb,
-- one_mm_pb, arena_pb) is sent client-side to the achiever only, via
-- create_notification, which hard-enforces `auth.uid() = p_user_id` — it
-- cannot safely be reused for a coach-facing feed (its own comment says so).
-- There is currently zero mechanism for a coach or admin to learn about a
-- client's achievement at all.
--
-- This adds a single SECURITY DEFINER helper, called from inside the five
-- submission RPCs that already compute the "is this a PB/tier-up" boolean
-- correctly server-side — the exact same shape those RPCs already use today
-- to notify a THIRD PARTY (the overtaken competitor, via the
-- leaderboard_overtaken inserts below), just fanning out to the achiever's
-- coach and that coach's active assistants instead.
--
-- Plain CREATE OR REPLACE on all five functions — no signature/return-type
-- changes, so no DROP FUNCTION needed anywhere.

-- Not exposed to authenticated/anon: only ever called from inside the
-- SECURITY DEFINER RPCs below, which already run as their owner.
CREATE OR REPLACE FUNCTION public.notify_coach_of_achievement(
  p_client_id uuid,
  p_type text,
  p_title text,
  p_body text,
...
BEGIN
  ...
  WHERE warrior_id = p_client_id AND status = 'active'
  ORDER BY assigned_at DESC LIMIT 1;

  IF v_coach_id IS NULL OR v_coach_id = p_client_id THEN
    RETURN; -- no assigned coach, or a coach's own test/demo account
  END IF;

  FOR v_recipient IN
    SELECT v_coach_id
    UNION
    SELECT assistant_id FROM public.coach_assistants WHERE coach_id = v_coach_id
  LOOP
    IF NOT EXISTS (
      SELECT 1 FROM public.notification_preferences
      WHERE user_id = v_recipient AND (prefs->>p_type) = 'false'
    ) THEN
      INSERT INTO public.notifications (user_id, type, title, body, data)
      VALUES (v_recipient, p_type, p_title, p_body, p_data);
    END IF;
  END LOOP;
END;
$function$;

REVOKE EXECUTE ON FUNCTION public.notify_coach_of_achievement(uuid, text, text, text, jsonb) FROM PUBLIC, anon, authenticated;

-- 1. Strength (submit_trial_result)
...
BEGIN
  ...
  IF v_tier_advanced THEN
    PERFORM public.notify_coach_of_achievement(
      v_user_id,
      'client_achievement',
      'Client Tier Up!',
      format('%s reached Tier %s in Strength.', COALESCE(v_profile.display_name, 'A warrior'), v_new_tier),
      jsonb_build_object('kind', 'tier_promotion', 'world', 'strength', 'new_tier', v_new_tier)
    );
  END IF;
...
BEGIN
    ...
    IF v_is_new_pb THEN
        PERFORM public.notify_coach_of_achievement(
            ...
            'client_achievement',
            'Client PB!',
            format('%s set a new Power World PB.', COALESCE(v_display_name, 'A warrior')),
            jsonb_build_object('kind', 'power_pb', 'world', 'power', 'tier', v_new_profile_tier)
        );
    END IF;
...
BEGIN
    ...
    IF v_is_new_pb THEN
        PERFORM public.notify_coach_of_achievement(
            ...
            'client_achievement',
            'Client PB!',
            format('%s set a new Static World PB.', COALESCE(v_display_name, 'A warrior')),
            jsonb_build_object('kind', 'static_pb', 'world', 'static', 'movement_id', p_movement_id, 'hold_seconds', p_hold_seconds)
        );
    END IF;
...
BEGIN
    ...
    IF v_is_new_pb THEN
        PERFORM public.notify_coach_of_achievement(
            ...
            'client_achievement',
            'Client PB!',
            format('%s set a new 1MM PB.', COALESCE(v_display_name, 'A warrior')),
            jsonb_build_object('kind', 'one_mm_pb', 'world', 'one_mm', 'movement_id', p_movement_id, 'reps', p_reps)
        );
    END IF;
...
BEGIN
  ...
  SELECT strength_tier, display_name INTO v_strength_tier, v_display_name
  FROM public.profiles
  WHERE id = v_user_id;
  ...
  IF v_is_new_best THEN
    PERFORM public.notify_coach_of_achievement(
      v_user_id,
      'client_achievement',
      'Client PB!',
      format('%s set a new Champions Arena PB.', COALESCE(v_display_name, 'A warrior')),
      jsonb_build_object('kind', 'arena_pb', 'world', 'arena', 'phase_id', p_phase_id, 'time_seconds', p_time_seconds)
    );
  END IF;
