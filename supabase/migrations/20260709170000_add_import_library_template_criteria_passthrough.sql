-- Lets a source import file that already includes matching_criteria /
-- equipment_tags apply them at insert time, instead of forcing the coach to
-- re-enter them via the filter editor every time. New trailing parameters
-- with defaults — existing callers passing only the first 3 args are
-- unaffected. status is still never taken from the file: this always
-- inserts as 'draft', same as before.
