-- Freemium gating redesign (2026-08-28): Program Templates keeps its
-- existing "exactly one recommended template is free" mechanic, but closes
-- a gap the client never enforced — nothing stopped a free user from
-- selecting a second (or third...) template, silently completing the first
-- as an unconditional side effect of every clone RPC's deactivate-then-
-- insert pattern. Now: any warrior_programs row ever created for a
-- non-Pro caller (active OR completed) blocks a further free selection —
-- "one free program, ever," confirmed with the product owner, not just
-- "no switching while one is currently active." Pro callers are
-- unaffected — v_is_pro short-circuits the new check exactly like every
...
BEGIN
    ...
    -- One free program, ever. Any prior warrior_programs row for this
    -- caller — regardless of status — means their one free pick is spent.
    IF NOT v_is_pro AND EXISTS (
        SELECT 1 FROM warrior_programs WHERE warrior_id = v_warrior_id
    ) THEN
