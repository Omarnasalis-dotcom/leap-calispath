-- Adds the well-rounded (WRA) piggyback check to submit_power_assessment,
-- completing the same pattern already applied to submit_static_hold
-- (20260805190000) and submit_onemm_log (20260805200000) — a power_points
-- change can move WRA rank too (statics_tier + power_points + one_mm_points).
...
BEGIN
    ...
    -- force bypass exists).
    SELECT COALESCE(power_points, 0.0), COALESCE(power_tier, 0), COALESCE(statics_tier, 0), COALESCE(one_mm_points, 0)
    INTO v_profile_points, v_profile_tier, v_statics_tier, v_one_mm_points
