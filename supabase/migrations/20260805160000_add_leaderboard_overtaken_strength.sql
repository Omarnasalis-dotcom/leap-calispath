-- Leaderboard-overtaken notifications, strength tier board first. When a
-- submission beats the caller's own previous best for that tier, find the
-- single competitor within the SAME tier (matching get_tier_leaderboard's
-- own per-tier scoping) whose best time is now worse than the caller's new
-- time but was better than or equal to the caller's old time — i.e. the
-- specific person the caller just leapfrogged, not a fan-out to everyone
-- passed. Notification is inserted directly here (SECURITY DEFINER bypasses
-- the self-only restriction on create_notification) so detection and
-- creation are atomic with the submission itself, not a separate trip.
...
BEGIN
  ...
  IF v_is_new_best THEN
    ...
    WHERE th.best_time > p_time_seconds AND th.best_time <= v_prev_best_time
