-- submit_power_assessment had no submission cooldown — unlike every other
-- submission RPC in this codebase (submit_trial_result: 30s in Edge Function,
-- submit_static_hold: 30s via static_hold_attempts, submit_onemm_log: 30s
-- via one_min_max_logs). A user could call it in a tight loop. The fix is
-- simpler than Static/1MM: since power assessments are all-four-lifts-at-once
-- with one row per user (power_assessments.assessed_at), a global per-user
-- cooldown can check that existing timestamp — no new table needed.
-- Also adds P1001/P1002 error codes for structured client-side handling,
-- matching the P1001-P1004 pattern on submit_static_hold and submit_onemm_log.
