-- assign_program_template trusted p_coach_id from client input with no check
-- that it matched the caller. A non-admin coach could pass any other coach's
-- id and silently reassign their client / force-complete their active program.
-- MyClientsScreen always passes the caller's own profile.id as coachId for
-- normal coaches, but its isAdmin mode (and the standalone /my-clients route,
-- which accepts an arbitrary coachId query param alongside isAdmin) requires
-- admins to be able to assign on behalf of any coach — so this is an
-- owner-or-admin check, not a strict auth.uid() = p_coach_id check.
