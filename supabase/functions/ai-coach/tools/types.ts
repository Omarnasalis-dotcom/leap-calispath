...
//
// No per-request context object (2026-09-17, day-by-day build): the old
// RequestContext/ProgramDraft existed only for add_program_day's whole-week
// staging, which no build path uses anymore — every propose_new_program/
// propose_add_day call is already exactly one day's blocks, so there is
// nothing left for a tool to accumulate across calls within one request.
