...
**Weighted Strength** — present in every program, matching the original methodology: a dedicated day in 4+ day splits, folded into Pull day in 3-day Foundation (§15). Never withheld pending a strength milestone — the skill hold is *their* real checkpoint from §8, whatever their line is actually at, and external load scales to what they can currently handle, same principle as the 20kg on Legs day: Dips and Pull Ups (Normal Grip) start banded/assisted if that is their real level and progress toward strict-and-loaded as logs show readiness, both is_weighted. Max effort ×3, 2min rest on the skill hold. Optional core, cool-down.
