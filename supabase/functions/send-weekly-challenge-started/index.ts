...
const NOTIFICATION_TYPE = "weekly_challenge_started";
...
Deno.serve(async (req) => {
  ...
  const { data: pending, error: pendingError } = await admin.rpc("get_pending_weekly_challenge_announcements");
  if (pendingError) {
    console.error("get_pending_weekly_challenge_announcements error:", pendingError);
  ...
  // backlog again (exactly what happened on 2026-08-05 — 33 stale
  // challenges were mass-announced to 160 real users in one run) — abort
  ...
  for (const challenge of pending as Array<{ challenge_id: string; title: string; group_id: number }>) {
    const { data: targets, error: targetsError } = await admin.rpc("get_weekly_challenge_target_users", {
      p_group_id: challenge.group_id,
      p_preference_key: NOTIFICATION_TYPE,
    });
    if (targetsError) {
      console.error("get_weekly_challenge_target_users error:", targetsError);
      continue;
    }
    ...
    const outcome = await notifyUsers(
      ...
      `"${challenge.title}" is live this week — get your score on the board.`,
      { screen: "weekly-challenge" }
    );

    await admin.from("weekly_challenges").update({ started_notification_sent_at: new Date().toISOString() }).eq("id", challenge.challenge_id);
